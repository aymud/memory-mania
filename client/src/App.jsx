...
// import StartGameInstructions from "./components/StartGameInstructions";
...
export default function App() {
  ...
  //   const [isStartGameInstruction, setIsStartGameInstruction] = useState(false);
  ...
  function handleTestSubmit() {
    ...
    const namesValidated = enteredNames.map((actualUser) => {
      const expectedUser = randomUsers.find(
        (user) => user.id.value === actualUser.id
      );
      if (expectedUser) {
        const isNameCorrect =
  ...
  return (
    <main>
      ...
      {isWaitingTestStart ? (
        ...
        <div className="user-cards-container">{randomUserElements}</div>
      )}
      {isLearningPhase && (
        <p className="test-countdown-container">
          {" "}
          {learningPhaseTimeRemainingInSeconds} seconds remaining
        </p>
      )}
      {isLearningPhase && (
        <button className="test-button" onClick={handleTestStart}>
          Test
        </button>
      )}
      {isGameStarted &&
        ...
        !isWaitingTestStart &&
        !isGameOver && (
          <button className="submit-button" onClick={handleTestSubmit}>
            Finish Test
          </button>
        )}
      {isGameOver && (
        <React.Fragment>
          ...
          />
          <button className="restart-button" onClick={handleGameRestart}>
            Restart Test
          </button>
        </React.Fragment>
      )}
